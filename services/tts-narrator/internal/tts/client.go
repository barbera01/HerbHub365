...
// Client sends text to the Kokoro TTS API and returns raw MP3 bytes.
type Client struct {
...
// request mirrors the JSON body expected by Kokoro FastAPI.
type request struct {
...
// Speak sends text to the TTS API using the client's configured voice.
func (c *Client) Speak(ctx context.Context, text string) ([]byte, error) {
	return c.SpeakAs(ctx, text, c.cfg.Voice)
}

// SpeakAs sends text to the TTS API using a specific voice name (friendly or raw).
func (c *Client) SpeakAs(ctx context.Context, text, voiceName string) ([]byte, error) {
	voice := ResolveVoice(voiceName, c.cfg.Speed)
